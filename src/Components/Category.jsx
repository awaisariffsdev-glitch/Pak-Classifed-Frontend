...
function Categories() {
    const navigate = useNavigate();
    const [visibleCards, setVisibleCards] = useState({}); // tracks which cards have animated in
    const cardRefs = useRef([]); // refs for each card to observe
        ...
        <div className="cat-section" id="categories-section">
            ...
            <div className="cat-grid">
                {categoryData.map((cat, index) => (
                    <div
                        key={cat.name}
                        ref={(el) => (cardRefs.current[index] = el)} // attach ref
                        data-index={index} // identify card in observer callback
                        className={`cat-card ${visibleCards[index] ? 'cat-card-visible' : ''}`} // conditional animation class
                        style={{ transitionDelay: `${(index % 4) * 80}ms` }} // staggered reveal per row
                        onClick={() => navigate(`/cars/category/${cat.name}`)}
                    >
                        <div className="cat-img-wrap">
                            <img src={cat.image} alt={cat.name} className="cat-img" />
            ...
            <style>{`
                ...
                .cat-card {
                    ...
                    cursor: pointer;
                    box-shadow: 0 2px 8px rgba(0, 0, 0, 0.06);
                    /* initial hidden state — invisible and shifted down before scrolling into view */
                    opacity: 0;
                    transform: translateY(40px);
                    transition: opacity 0.6s ease, transform 0.6s ease, box-shadow 0.2s ease;
                }
                ...
                .cat-card-visible {
                    /* final revealed state — fades in and slides up smoothly */
                    opacity: 1;
                    transform: translateY(0);
                }
                ...
                @media (max-width: 480px) {
                    .cat-grid { grid-template-columns: 1fr 1fr; gap: 14px; }
                    /* CHANGED: 14px -> 16px so this matches CarBoardMain's
                       .cbm-container mobile side padding exactly */
